...
Create a cluster on [MongoDB Atlas](https://www.mongodb.com/cloud/atlas), then:
- Go to **Database Access** → create a database user (username + password)
- Go to **Network Access** → add your IP (or `0.0.0.0/0` for development)
...
Deploy the frontend and backend as **two separate Vercel projects** from the same GitHub repo.
...
> **Note:** Vercel uses serverless functions. Local file uploads (images/videos) are **not reliable** on Vercel because the filesystem is temporary. For production uploads, use [Cloudinary](https://cloudinary.com) or deploy the backend on [Render](https://render.com) instead.
...
- Add `0.0.0.0/0` in **Network Access** so Vercel can connect
