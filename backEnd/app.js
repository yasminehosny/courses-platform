...
// debug: show whether MONGO_URI is loaded (do not log secrets in production)
console.log("MONGO_URI (loaded):", process.env.MONGO_URI ? "<present>" : "<missing>");

const allowedOrigins = process.env.FRONTEND_URL
  ? process.env.FRONTEND_URL.split(",").map((origin) => origin.trim())
  : ["http://localhost:5173", "https://courses-platform-ten.vercel.app"];

app.use(
  cors({
    origin: function (origin, callback) {
      // Allow all origins dynamically to fix Vercel CORS issues
      callback(null, true);
    },
    credentials: true,
  })
