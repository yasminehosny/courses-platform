...
export const getMyEnrollments = async (req, res, next) => {
  try {
    ...
    // Enrich each enrollment with totalLessons count for progress calculation
    const enriched = await Promise.all(
      enrollments.map(async (e) => {
        const obj = e.toObject();
        if (e.courseID?._id) {
          obj.totalLessons = await Lesson.countDocuments({ courseID: e.courseID._id });
        } else {
          obj.totalLessons = 0;
        }
        return obj;
    ...
    res.status(200).json({ enrollments: enriched });
...
export const getCourseProgress = async (req, res, next) => {
  try {
    ...
    const enrollment = await Enrollment.findOne({ studentID, courseID: courseId });
    if (!enrollment) return next(new HttpError(404, "Not enrolled"));
